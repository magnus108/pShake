...
dropdown
    ...
    -> UI
           ( Element
           , Behavior (ListZipper.ListZipper a -> [UI Element])
           , Behavior (ListZipper.ListZipper a -> [UI Element])
           , Tidings Mode
...
dropdown bDisplayClosed bDisplayOpen = mdo
    ...
    let eSwitch = switch <$> Unsafe.head <$> unions
            [bDropMode <@ eSelection, bDropMode <@ ePopup]

    bDropMode <- stepper Closed $ eSwitch
    let tDropMode = tidings bDropMode eSwitch

    let bDisplayOpen' =
            bDisplayOpen <&> \f b (zipper :: ListZipper.ListZipper a) -> do
    ...
    let bDisplayClosed' =
            bDisplayClosed <&> \f (zipper :: ListZipper.ListZipper a) -> do
    ...
    let bDisplayOpen'' =
            (\f z -> ListZipper.toList (ListZipper.bextend f z))
                <$> bDisplayOpen'
...
dropdown3 bDisplayClosed bDisplayOpen = mdo
    ...
    let bView = do
            ...
            return $ \zipper -> case dropMode of
                Open -> do
